...
class HeathSign(object):
    ...
    def check_cookies(self):
        ...
        if "cookies.txt" not in os.listdir(COOKIES_PATH):
            with open(COOKIES_PATH + "/cookies.txt", "w+") as f:
                ...
                r = requests.get(
                    'https://docs.qq.com/dop-api/opendoc?normal=1&id={}'.format(id))
    ...
    def run(self):
        ...
        if not self.check_cookies():
            msg = {
                'msg': False,
                'detail': '打卡失败，原因：cookies已失效，请及时更新！\r\r1. 在文档内粘贴新的cookies，然后访问下方链接进行远程更新\r\r{}\r\r2. 再访问下方链接进行手动打卡{}'.format(
